...
+(instancetype)sharedInstance{
    static EWSInboxList* instnace = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instnace = [[EWSInboxList alloc] init];
    });
    return instnace;
}
...
-(void)getInboxListWithEWSUrl:(NSString *)url resultBlock:(void(^)(NSMutableArray *inboxList, NSError *error))getInboxListBlock{
    ...
    [request ewsHttpRequest:soapXmlString andUrl:url emailBoxInfo:((EWSService *)[EWSService sharedInstance]).ewsEmailBoxModel receiveResponse:^(NSURLResponse *response) {
